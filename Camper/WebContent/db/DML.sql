...
-- 3. 전체 캠핑장 목록
SELECT S_CAMP_NAME, S_CAMP_ADDR, S_CAMP_MAINPIC FROM HOST_CAMPGROUND HC, MEMBER_HOST MH WHERE HC.S_HID = MH.S_HID WHERE MH.H_DEL_YN = 'N' ORDER BY CG_RDATE DESC;
...
INSERT INTO GUEST_RESERVATION 
    VALUES (TO_CHAR(SYSDATE, 'YYMMDD')||'-'||(TO_CHAR(GUEST_RESERVATION_NO_SEQ.NEXTVAL)), '111111', '2023-03-31', '2023-04-01', 'testguest', SYSDATE, 'N');
...
-- 예약 수
SELECT COUNT(*) CNT FROM GUEST_RESERVATION WHERE S_SITE_NO = '111111';
...
-- 3. 예약 수정                
UPDATE GUEST_RESERVATION SET S_SITE_NO = '111111',
                            D_REZ_FROM = TO_CHAR('2023-03-30'),
                            D_REZ_TO = TO_CHAR('2023-04-01')
                        WHERE S_REZ_NO = '230321-111112';
-- 4. 예약 삭제
...
INSERT INTO HOST_CAMPGROUND 
    VALUES ('CG'||TO_CHAR(HOST_CAMPGROUND_NO_SEQ.NEXTVAL), '산마루캠핑장', '산마루캠핑장입니다', '경남 산청군 시천면 지리산대로1478번길 31-10', 'host', 'main.jpg', 'map.jpg', 'photo1.jpg', 'photo2.jpg', NULL, NULL, NULL, 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', SYSDATE);
...
-- 5. 등록된 내 캠핑장 목록
SELECT S_CAMP_NO, S_CAMP_NAME FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM HOST_CAMPGROUND) A, MEMBER_HOST MH WHERE A.S_HID = MH.S_HID AND A.S_HID = 'host' AND MH.H_DEL_YN = 'N') WHERE RN BETWEEN 1 AND 2;
...
-- 4. 캠핑 사이트 목록
SELECT S_SITENAME, S_SITEPRICE FROM HOST_CAMPSITE HCS, HOST_CAMPGROUND HC, MEMBER_HOST MH WHERE HCS.S_CAMP_NO = HC.S_CAMP_NO AND MH.H_DEL_YN = 'N' AND HC.S_HID = MH.S_HID AND HC.S_CAMP_NO = '111111' ORDER BY CS_RDATE;
