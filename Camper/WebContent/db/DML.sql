...
INSERT INTO GUEST_RESERVATION 
    VALUES (TO_CHAR(SYSDATE, 'YYMMDD')||'-'||(TO_CHAR(GUEST_RESERVATION_NO_SEQ.NEXTVAL)), 'CS111111', TO_CHAR(TO_DATE('2023-03-30', 'YYYY-MM-DD')), 'testguest', SYSDATE, 'N');
...
-- 캠핑장 수락한 예약 수
SELECT GR.* FROM GUEST_RESERVATION GR, HOST_CAMPGROUND HC, HOST_CAMPSITE HS WHERE HS.S_SITE_NO = GR.S_SITE_NO AND HC.S_CAMP_NO =  HS.S_CAMP_NO AND HC.S_CAMP_NO = 'CG111111' AND GR.GR_STATUS = 'Y';

-- 캠핑장 수락하지 않은 예약 수
SELECT GR.* FROM GUEST_RESERVATION GR, HOST_CAMPGROUND HC, HOST_CAMPSITE HS WHERE HS.S_SITE_NO = GR.S_SITE_NO AND HC.S_CAMP_NO =  HS.S_CAMP_NO AND HC.S_CAMP_NO = 'CG111111' AND GR.GR_STATUS = 'N';

-- 예약 수락 여부
SELECT GR.* FROM GUEST_RESERVATION GR WHERE S_SITE_NO = 'CS111111' AND D_SELECT LIKE TO_CHAR(TO_DATE('2023-03-30', 'YYYY-MM-DD'));
SELECT GR.* FROM GUEST_RESERVATION GR;
-- 1. 예약 확인
SELECT GR.*, S_GTEL FROM GUEST_RESERVATION GR, MEMBER_GUEST MG WHERE GR.S_GID = MG.S_GID AND S_SITE_NO = 'CG111111' ORDER BY D_REZ_DATE;
...
-- 해당 년월에 예약된 내용 보기
SELECT * FROM GUEST_RESERVATION WHERE S_SITE_NO = 'CS111111' AND TO_CHAR(D_SELECT, 'YYYY-MM') LIKE '2023' || '-' || '03';
