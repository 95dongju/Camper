...
public class ReservationDao {
	...
	public int reservation(ReservationDto rez) {
		...
		String sql = "INSERT INTO GUEST_RESERVATION " + 
				"    VALUES (TO_CHAR(SYSDATE, 'YYMMDD')||'-'||(TO_CHAR(GUEST_RESERVATION_NO_SEQ.NEXTVAL)), ?, TO_DATE(?, 'YYYY-MM-DD'), ?, SYSDATE, 'N')";
		try {
	...
	public int getReservation(String s_site_no, String selectDate){
		...
		PreparedStatement pstmt = null;
		ResultSet         rs    = null;
		String sql = "SELECT GR.* FROM GUEST_RESERVATION GR WHERE S_SITE_NO = ? AND D_SELECT LIKE TO_DATE(?, 'YYYY-MM-DD')";
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
